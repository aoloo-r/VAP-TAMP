...
class RobotAgent:
    ...
    def __init__(
        ...
        self._is_safe_threshold = self.parameters["motion_planner"].get("is_safe_threshold", 1.0)
        ...
        # Pass robot footprint parameters directly to the navigation space
        nav_kwargs = {
        ...
        # Use a validation function that includes the is_safe_threshold parameter
        def validation_fn(state):
            return self.space.is_valid(state, is_safe_threshold=self._is_safe_threshold, verbose=False)

        self.planner: Planner = get_planner(
            self.parameters.get("motion_planner/algorithm", "rrt_connect"),
            self.space,
            validation_fn,
            max_iter=self.parameters.get("motion_planner/max_iter", 100),
        )
    ...
    def plan_to_instance(
        ...
        # Check if start is valid, if not try to find a nearby valid position
        start_is_valid = self.space.is_valid(start, is_safe_threshold=self._is_safe_threshold, verbose=False)
        if not start_is_valid:
            print(f"🔧 Start position {start} is invalid, searching for nearby valid position...")
            # Try positions in a small radius around the start
            for radius in [0.1, 0.2, 0.3, 0.5]:
                for angle in [0, np.pi/4, np.pi/2, 3*np.pi/4, np.pi, 5*np.pi/4, 3*np.pi/2, 7*np.pi/4]:
                    ...
                    test_start[0] += radius * np.cos(angle)
                    test_start[1] += radius * np.sin(angle)
                    if self.space.is_valid(test_start, is_safe_threshold=self._is_safe_threshold, verbose=False):
                        print(f"✅ Found valid start position: {test_start} (offset by {radius:.1f}m)")
                        start = test_start
                        start_is_valid = True
                        break
                if start_is_valid:
            ...
            if not start_is_valid:
                print(f"❌ Could not find valid start position near {start}")
    ...
    def plan_to_bounds(
        ...
        # Try conservative sampling first, then fallback to non-conservative if needed
        sampled_goals = 0
        valid_goals = 0
        successful_plans = 0
        
        # First try conservative sampling (staying away from obstacles and edges)
        print(f"🔍 Trying conservative sampling near target...")
        goals_tried = 0
        for goal in self.space.sample_near_mask(
            ...
            show_goal[:2] = goal[:2]
            goal_is_valid = self.space.is_valid(goal, is_safe_threshold=self._is_safe_threshold, verbose=False)
            if goal_is_valid:
                valid_goals += 1
            if verbose:
            ...
            if res.success or try_count > max_tries:
                break
            # Conservative sampling only - no fallback to non-conservative
            if try_count > max_tries:
                break

        # Planning failed
        print(f"🔍 Navigation planning stats: sampled_goals={sampled_goals}, valid_goals={valid_goals}, successful_plans={successful_plans}")
        if res is None:
    ...
    def recover(self):
        ...
        for step_i in range(5, 100, 5):
            ...
            if self.space.is_valid(start, is_safe_threshold=self._is_safe_threshold, verbose=True):
            ...
            if self.space.is_valid(xyt_goal_backward, is_safe_threshold=self._is_safe_threshold, verbose=True):
            ...
            elif self.space.is_valid(xyt_goal_forward, is_safe_threshold=self._is_safe_threshold, verbose=True):
    ...
    def recover_from_invalid_start(self, verbose: bool = True) -> bool:
        ...
        while max_tries > 0:
            ...
            start = self.robot.get_base_pose()
            start_is_valid = self.space.is_valid(start, is_safe_threshold=self._is_safe_threshold, verbose=True)
    ...
    def go_home(self):
        ...
        start = self.robot.get_base_pose()
        goal = np.array([0, 0, 0])
        print(f"- Current pose is valid: {self.space.is_valid(self.robot.get_base_pose(), is_safe_threshold=self._is_safe_threshold)}")
        print(f"-   start pose is valid: {self.space.is_valid(start, is_safe_threshold=self._is_safe_threshold)}")
        print(f"-    goal pose is valid: {self.space.is_valid(goal, is_safe_threshold=self._is_safe_threshold)}")

        # If the start is invalid, try to recover
        if not self.space.is_valid(start, is_safe_threshold=self._is_safe_threshold):
    ...
    def plan_to_frontier(
        ...
        if random_goals:
            ...
            for i, goal in enumerate(sampler):
                ...
                if self.space.is_valid(goal.cpu().numpy(), is_safe_threshold=self._is_safe_threshold, verbose=True):
                    if verbose:
                        print("       Start:", start)
    ...
    def get_object_centric_observations(
        ...
        world_representation = self.get_object_centric_world_representation(
            ...
            max_context_length=int(self.parameters.get("vlm/vlm_context_length", 20)),
